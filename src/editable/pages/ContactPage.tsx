    ...
    <EditableSiteShell>
      <main className="editable-section bg-[#111] text-white">
        <section className="border-b border-white/10">
          <div className="mx-auto max-w-[1340px] px-4 py-14 sm:px-6 lg:px-8 lg:py-20">
            <p className="text-xs font-black uppercase tracking-[0.28em] text-[var(--slot4-accent)]">{pagesContent.contact.eyebrow}</p>
            <h1 className="mt-4 max-w-5xl text-5xl font-black leading-tight sm:text-7xl">{pagesContent.contact.title}</h1>
            <p className="mt-6 max-w-2xl border-l-4 border-[var(--slot4-accent)] pl-5 text-base font-semibold leading-8 text-white/65">{pagesContent.contact.description}</p>
          </div>
        </section>

        <section className="mx-auto grid max-w-[1340px] border-x border-white/15 bg-[#141414] lg:grid-cols-[0.72fr_1.28fr]">
          <aside className="border-b border-white/15 bg-[#101010] text-white lg:border-b-0 lg:border-r lg:border-white/15">
